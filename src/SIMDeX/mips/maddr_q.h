#if !defined(SIMDE_MIPS_MSA_MADDR_Q_H)
#define SIMDE_MIPS_MSA_MADDR_Q_H
...
simde_v8i16
simde_msa_maddr_q_h(simde_v8i16 a, simde_v8i16 b, simde_v8i16 c) {
  #if defined(SIMDE_MIPS_MSA_NATIVE)
    return __msa_maddr_q_h(a, b, c);
  #else
    simde_v8i16_private
    ...
    for (size_t i = 0 ; i < (sizeof(r_.values) / sizeof(r_.values[0])) ; i++) {
      ...
      int16_t q = (p == INT32_C(0x40000000)) ? INT16_MAX : HEDLEY_STATIC_CAST(int16_t, (p + 16384) >> 15);
  ...
  #undef __msa_maddr_q_h
  #define __msa_maddr_q_h(a, b, c) simde_msa_maddr_q_h((a), (b), (c))
#endif

SIMDE_FUNCTION_ATTRIBUTES
simde_v4i32
simde_msa_maddr_q_w(simde_v4i32 a, simde_v4i32 b, simde_v4i32 c) {
  #if defined(SIMDE_MIPS_MSA_NATIVE)
    return __msa_maddr_q_w(a, b, c);
  #else
    simde_v4i32_private
    ...
    for (size_t i = 0 ; i < (sizeof(r_.values) / sizeof(r_.values[0])) ; i++) {
      ...
      int32_t q = (p == INT64_C(0x4000000000000000)) ? INT32_MAX : HEDLEY_STATIC_CAST(int32_t, (p + INT64_C(1073741824)) >> 31);
...
#if defined(SIMDE_MIPS_MSA_ENABLE_NATIVE_ALIASES)
  #undef __msa_maddr_q_w
  #define __msa_maddr_q_w(a, b, c) simde_msa_maddr_q_w((a), (b), (c))
#endif
