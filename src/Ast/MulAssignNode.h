...
namespace ve {
	...
	/**
	 * Represents a compound multiplication assignment operation (*=).
	 **/
	class MulAssignNode : public AstNode {
	public :
		MulAssignNode(size_t var, size_t right) : varIndex(var), rightIndex(right) {}


		// == Functions.
		/**
		 * Evaluates the compound multiplication assignment of the variable and right child node.
		 * 
		...
		Result						evaluate(ExecutionContext& context) const override {
			...
			Result out;
			out = context.evaluateMath(leftVal, rightVal, ExprLexer::MUL_ASSIGN);
		 ...
		 **/
		virtual NodeType			type() const { return NodeType::MulAssign; }
