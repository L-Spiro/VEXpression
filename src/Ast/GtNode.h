...
namespace ve {
	...
	/**
	 * Represents a greater-than comparison operation between two child nodes.
	 **/
	class GtNode : public AstNode {
	public :
		GtNode(size_t left, size_t right) : leftIndex(left), rightIndex(right) {}

		
		// == Functions.
		/**
		 * Evaluates whether the left child node is strictly greater than the right.
		 * 
		 ...
		 **/
		virtual NodeType			type() const { return NodeType::Gt; }
