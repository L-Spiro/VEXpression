...
namespace ve {
	...
	/**
	 * Represents a less-than-or-equal comparison operation between two child nodes.
	 **/
	class LeNode : public AstNode {
	public :
		LeNode(size_t left, size_t right) : leftIndex(left), rightIndex(right) {}

		
		// == Functions.
		/**
		 * Evaluates whether the left child node is less than or equal to the right.
		 * 
		 ...
		 **/
		virtual NodeType			type() const { return NodeType::Le; }
