...
namespace ve {
	...
	/**
	 * Represents a less-than comparison operation between two child nodes.
	 **/
	class LtNode : public AstNode {
	public :
		LtNode(size_t left, size_t right) : leftIndex(left), rightIndex(right) {}

		
		// == Functions.
		/**
		 * Evaluates whether the left child node is strictly less than the right.
		 * 
		 ...
		 **/
		virtual NodeType			type() const { return NodeType::Lt; }
