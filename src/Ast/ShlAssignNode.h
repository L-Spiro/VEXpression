...
namespace ve {
	...
	/**
	 * Represents a compound bitwise shift left assignment operation (<<=).
	 **/
	class ShlAssignNode : public AstNode {
	public :
		ShlAssignNode(size_t var, size_t right) : varIndex(var), rightIndex(right) {}


		// == Functions.
		/**
		 * Evaluates the compound bitwise shift left assignment of the variable and right child node.
		 * 
		...
		Result						evaluate(ExecutionContext& context) const override {
			...
			Result out;
			out = context.evaluateMath(leftVal, rightVal, ExprLexer::SHL_ASSIGN);
		 ...
		 **/
		virtual NodeType			type() const { return NodeType::ShlAssign; }
