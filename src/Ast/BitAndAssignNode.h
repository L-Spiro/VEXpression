...
namespace ve {
	...
	/**
	 * Represents a compound bitwise AND assignment operation (&=).
	 **/
	class BitAndAssignNode : public AstNode {
	public :
		BitAndAssignNode(size_t var, size_t right) : varIndex(var), rightIndex(right) {}


		// == Functions.
		/**
		 * Evaluates the compound bitwise AND assignment of the variable and right child node.
		 * 
		...
		Result						evaluate(ExecutionContext& context) const override {
			...
			Result out;
			out = context.evaluateMath(leftVal, rightVal, ExprLexer::AND_ASSIGN);
		 ...
		 **/
		virtual NodeType			type() const { return NodeType::BitAndAssign; }
